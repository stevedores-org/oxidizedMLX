...
mod tests {
    ...
    fn test_attention_block() {
        ...
        let q = Tensor::from_f32_on_stream(
            &q_data,
            &Shape::new(vec![seq_len as i64, d_k as i64]),
            &stream,
        )
        .unwrap();
        ...
        let k_data: Vec<f32> = (0..seq_len * d_k)
            .map(|i| (i as f32 + 10.0) / 100.0)
            .collect();
        let k = Tensor::from_f32_on_stream(
            &k_data,
            &Shape::new(vec![seq_len as i64, d_k as i64]),
            &stream,
        )
        ...
        let v_data: Vec<f32> = (0..seq_len * d_k)
            .map(|i| (i as f32 + 20.0) / 100.0)
            .collect();
        let v = Tensor::from_f32_on_stream(
            &v_data,
            &Shape::new(vec![seq_len as i64, d_k as i64]),
            &stream,
        )
        ...
        let scale_tensor = Tensor::from_f32_on_stream(
            &scale_data,
            &Shape::new(vec![seq_len as i64, seq_len as i64]),
            &stream,
        )
        .unwrap();
